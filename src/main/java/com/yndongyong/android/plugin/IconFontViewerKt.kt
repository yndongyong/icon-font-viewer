...
import com.yndongyong.android.util.Notifier
...
class IconFontViewerKt(var project: Project, var ttfFile: VirtualFile) : IconFontViewer() {
    ...
    private fun setUI() {
        try {
            ...
            jListIcons.addMouseListener(object :MouseAdapter(){
                override fun mouseClicked(e: MouseEvent) {
                    val index = jListIcons.locationToIndex(e.point)
                    val vo = listModel.get(index)
                    val uniCodeStr = "\\u" + Integer.toHexString(vo.codePoint.code)
                    CopyPasteManager.getInstance().setContents(StringSelection("yynui_${vo.postScript}"))
                    val notification = Notification("custom group id",
                        "IconFont: yynui_${vo.postScript}[${uniCodeStr}] copy success!!", NotificationType.INFORMATION)
